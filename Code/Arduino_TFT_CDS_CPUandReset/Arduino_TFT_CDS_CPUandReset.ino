...
void loop() {
  ...
  if (TFT_sleep_bool==false || CDS_A>TFT_sleep || CDS_B>TFT_sleep || CDS_C>TFT_sleep)
  {
    ...
    CLKPR = 1 << CLKPCE;
    CLKPR = 0;// 將CPU速度設定為 16MHz/1   Set the CPU speed to 16 MHz / 1
    sun_moon_display();
    Quokka_BOBO_display();

  if(value0 > 60 && Gift_bool == true ) //Touch A0
  {
  Gift_display();
  }
  ...
  else if (TFT_sleep_bool==true && CDS_A<=TFT_sleep && CDS_B<=TFT_sleep && CDS_C<=TFT_sleep)
  {
    digitalWrite(3,LOW);
    CLKPR = 1 << CLKPCE;
    CLKPR = 8;// 將CPU速度設定為 62.5KMHz/256  Set the CPU speed to 62.5 KHz / 256
  }
  wdt_reset();
  }
